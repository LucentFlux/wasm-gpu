use super::{basic_block_gen::BasicBlockState, BuildError};
use crate::module::operation::MVPOperator;

pub(super) fn eat_mvp_operator(
    state: &mut BasicBlockState<'_>,
    operator: &MVPOperator,
) -> Result<(), BuildError> {
    match operator {
        MVPOperator::Nop => { /* Pass */ }
        MVPOperator::I32Const { value } => {
            let val = state.constant(crate::Val::I32(*value));
            state.push(naga::Expression::Constant(val))
        }
        MVPOperator::I64Const { value } => {
            let val = state.constant(crate::Val::I64(*value));
            state.push(naga::Expression::Constant(val))
        }
        MVPOperator::F32Const { value } => {
            let val = state.constant(crate::Val::F32(crate::typed::Ieee32::from(*value)));
            state.push(naga::Expression::Constant(val))
        }
        MVPOperator::F64Const { value } => {
            let val = state.constant(crate::Val::F64(crate::typed::Ieee64::from(*value)));
            state.push(naga::Expression::Constant(val))
        }
